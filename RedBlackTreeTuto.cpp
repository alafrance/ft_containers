//
// Created by Alexis Lafrance on 11/26/21.
//
...
private:
	...
	void deleteFix(NodePtr x) {
		NodePtr s;
		std::cout << "x : " << x->parent->data << std::endl;
		std::cout << "1" << std::endl;
		while (x != root && x->color == 0) {
			...
			if (x == x->parent->left) {
				...
				std::cout << "5" << std::endl;
				if (s->left->color == 0 && s->right->color == 0) { // MISTAKES IN CODE
					std::cout << "6" << std::endl;
					s->color = 1;
					x = x->parent;
				} else {
	...
	void rbTransplant(NodePtr uncle, NodePtr node) {
